...
def draworbit(planet_turtle, planet, i, speed):

        planet_turtle.st()

        # coordinates
        planet_turtle.goto(coor(planet, i, speed)[0],
                           coor(planet, i, speed)[1])
...
def main():
    ...
    wn = turtle.Screen()
    wn.bgcolor("Black")
    speed = int(turtle.numinput('speed control','slowest = 1, fastest = 10',default=None,minval=1,maxval=10))
    ...
    for x in turtles:
        ...
        if j in range(1,8):
            x.ht()
            x.goto(coor(planet_list[turtles.index(x)], j, speed)[0],
                   coor(planet_list[turtles.index(x)], j, speed)[1])
        ...
        # rotation
        # x.left(planet_list[turtles.index(x)][4] + 
               # planet_list[turtles.index(x)][5])
    ...
    while True:
        ...
        draworbit(turtles[j % 8],
                  planet_list[j % 8], j, speed)
            
        # time parameter
        if b.xcor()<0 and b.ycor()<0 and b_lastxcoor<0 and b_lastycoor>0:
            ...
            text_turtle.color("White")
            text_turtle.write(str(int((time.clock() - time_start) *day_parameter)) + " days",
                              move=False, align="left",
                              font=("Arial", 24, "normal"))
