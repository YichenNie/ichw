...
def basetile(m, n, a, b):
    ...
    if bati1[-1] < m * n and len(list(set(bati1))) == len(bati1):  # 不加最后一个判断条件，运行(2, 3, 2, 3)给出(0, 1, 2, 2, 3, 4)
        bati_lst.append(bati1)
    if bati2[-1] < m * n and bati1 != bati2 and len(list(set(bati2))) == len(bati2):
        bati_lst.append(bati2)
    
    return bati_lst


# 墙面大小为m*n，在剩余空位（集合empty）中
...
def valid(empty, coor0, z, m, n):
    ...
    v = coor0 % m - z[0] % m
    for i in z:
        if not (i + coor0 in empty and (i + coor0) % m - i % m == v):  # 4*3的墙面上，不能用1块砖覆盖7和8
            tf = False
    ...
    if empty == set():  #  所有砖块填满时停止递归
        ...
        for v in bati:  # 两个方向或一个方向
            ...
            if valid(empty1, e, v, m, n) == True: # 如果可以填入bati[0]方向砖块
                ...
                occ1.add(tuple(j + e for j in v))  # 填入砖块
                for j in v:
...
def main():
    ...
    print('Input m, n, a, b. Use a blank space to separate each parameter.')
    ...
    with open('tile_' + str(m) + '_' + str(n) + '_' + str(a) + '_' + str(b) + '.txt', 'w') as tiletxt:
